...
public class Test {
	...
	public static void main(String[] args) {
		...
		System.out.println("The first part of the collection:");
	...
	*/
	 ...
	 * without repeating the elements that already exist in c1
	...
	*/
	...
	/*
	 * Generic method that combines the elements of two collections c1, and c2
	 * without repeating the elements that already exist in c1, recursively
	 * @param c1 the first collection
	 * @param c2 the second collection
	 * @return no return  value - the result is in c1
	 * O(n^2)
	*/
	public static <E> void recursiveCombineNoDuplicates(Collection<E> c1, Collection<E> c2) {
		...
		if(iter.hasNext()) {
			...
			recursiveCombineNoDuplicates(c1,c3);
		}		
	}
	...
	public static <E> void subCollection(Collection<E> c, int start,int end, Collection<E> subC) throws ArrayIndexOutOfBoundsException{
		...
		for(int i = 0; i < start; i++) {
			E before = (E)iter.next();
		}
		for(int i = start; i < end; i++) {
			subC.add((E)iter.next());
		}
	}
	 ...
	 * @param c1 the collection that will contain the elements
	 * from the first to delimiter (inclusive)
	 * @param c2 the collection that will contain the elements
